...
功能图标使用 `lucide_icons_flutter`：

- 页面导航、按钮、设置项等通用操作图标统一使用 Lucide。
- 不直接混用多套图标库，避免风格不一致。
- 如果 Lucide 没有合适图标，再考虑自定义 SVG。

心情贴纸使用项目内原创 SVG：

- 资源目录：`assets/stickers/`。
- 加载方式：`flutter_svg`。
- 贴纸入口组件：`MoodSticker`。
- `MoodOption` 只保存贴纸路径、颜色和语义信息，不在页面里硬编码资源路径。

版权原则：

- 不复制 Suki 或其他 App 的美术资源。
- 可以参考“柔和、贴纸化、情侣日历”的设计方向，但具体图形必须原创或使用明确授权的素材。
